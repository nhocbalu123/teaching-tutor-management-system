 ...
 */
    ...
    static async seedEssentialData(): Promise<void> {
        try {
            ...
            await this.seedMockCandidatesAndApplications();
    ...
    private static async seedMockCandidatesAndApplications(): Promise<void> {
        try {
            const bcrypt = await import("bcryptjs");
            const userRepository = AppDataSource.getRepository(User);
            const courseRepository = AppDataSource.getRepository(Course);
            ...
            const { UserType } = await import("../entities/User");
            const { ApplicationStatus } = await import("../entities/Application");
            const saltRounds = 10;
            ...
            for (const candidateData of mockCandidates) {
                ...
                if (!existingCandidate) {
                    const hashedPassword = await bcrypt.default.hash(candidateData.password, saltRounds);
